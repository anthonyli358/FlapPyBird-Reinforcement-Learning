...
def _pipes_from(lower_pipes):
    ...
    return [(p["x"], p["y"] - PIPEGAPSIZE // 2)
            for p in lower_pipes if p["x"] < SCREENWIDTH]
...
def simulate(R, mode, frames, seed, record=False, on_step=None):
    """Run the real game under a proposal policy, shielded by kernel ``R``.

    Returns dict: survived, died_at, score, overrides, frames_run, and (if
    ``record``) a trajectory list of per-frame dicts for the how-it-works figure.
    ``on_step(y, vel, proposal, action)`` is called each frame if given (used to
    tally the override heatmap without storing a full trajectory).
    """
    ...
    upperPipes = [{"x": SCREENWIDTH + 200, "y": p1["upper_y"]},
                  {"x": SCREENWIDTH + 200 + (SCREENWIDTH // 2), "y": p2["upper_y"]}]
    lowerPipes = [{"x": SCREENWIDTH + 200, "y": p1["lower_y"]},
    ...
    for f in range(frames):
        ...
        if record:
            ...
            traj.append({
                "f": f, "world_x": PLAYER_X + world0, "y": playery, "vel": playerVelY,
                "action": action, "proposal": proposal, "override": overrode,
                "in_kernel": bool(V.in_kernel(R, playery, playerVelY)),
                "pipes": [(p["x"] + world0, p["y"] - PIPEGAPSIZE, p["y"]) for p in lowerPipes],
                "screen_pipes": [(p["x"], p["y"] - PIPEGAPSIZE, p["y"]) for p in lowerPipes],
                "gov": gov[0] if gov else None, "score": score,
            })
        ...
        if _collides(playery, lowerPipes):
            return {"survived": False, "died_at": f, "score": score,
                    "overrides": overrides, "frames_run": f, "traj": traj}
    ...
    return {"survived": True, "died_at": None, "score": score,
            "overrides": overrides, "frames_run": frames, "traj": traj}
...
def fig_kernel(R):
    ...
    vmin = V.V_MIN
    extent = [vmin - 0.5, V.V_MAX + 0.5, V.Y_MAX + 0.5, -0.5]  # y inverted (down = down)
    ax.imshow(R.astype(float), aspect="auto", extent=extent,
              cmap=matplotlib.colors.ListedColormap(["#EEEEEE", C_SAFE]),
              interpolation="nearest")
    ax.axhspan(C.GAP_CENTER_MIN, C.GAP_CENTER_MAX, xmin=0, xmax=1,
               color=C_OK, alpha=0.10, lw=0)
    ax.axhline(C.GAP_CENTER_MIN, color=C_OK, lw=1, ls="--", alpha=0.7)
    ax.axhline(C.GAP_CENTER_MAX, color=C_OK, lw=1, ls="--", alpha=0.7)
    ax.text(V.V_MAX - 0.3, (C.GAP_CENTER_MIN + C.GAP_CENTER_MAX) / 2,
            "gap-centre band", color=C_OK, ha="right", va="center", fontsize=9)
    ys, _ = np.where(R)
    ax.set_title(f"Viability kernel R  (|R| = {int(R.sum())} / {R.size} states, "
                 f"y ∈ [{ys.min()}, {ys.max()}])", fontsize=11)
    ax.set_xlabel("bird velocity (px/frame)")
    ax.set_ylabel("bird y, top-left (px, down ↓)")
    ax.set_xticks(range(vmin, V.V_MAX + 1, 2))
    handles = [Rectangle((0, 0), 1, 1, color=C_SAFE),
               Rectangle((0, 0), 1, 1, color="#EEEEEE")]
    ax.legend(handles, ["safe (in R): can thread any next gap and return to R",
...
def fig_overrides(R, frames, seeds):
    ...
    labels = {"noflap": "always\nno-flap", "adversary": "adversary",
              "random": "random", "flap": "always\nflap"}
    rates, deaths = [], []
    for m in modes:
        rs, dz = [], 0
        for s in seeds:
    ...
    for b, r, d in zip(bars, rates, deaths):
        ...
        ax.text(b.get_x() + b.get_width() / 2, 3,
                ...
                color="white" if r > 12 else C_OK, fontweight="bold")
    ax.set_ylim(0, 100)
    ax.set_ylabel("frames the shield overrode the proposal (%)")
    ax.set_title(f"The worse the proposal, the more the shield steps in — "
                 f"survival is unconditional\n"
                 f"({len(seeds)} seeds × {frames:,} frames each)", fontsize=10)
    fig.tight_layout()
...
def fig_phase_fix(R_robust, frames, seeds, single_cap=60000):
    ...
    for v in single_v:
        ax.annotate(f"{v}", (0, v), textcoords="offset points", xytext=(11, 0),
                    fontsize=8, va="center", color="#7a1f1f")
    ax.annotate(f"{n_died}/{len(seeds)} seeds die\n(median {int(np.median(single_v))} pipes)",
                (0, np.mean(single_v)), textcoords="offset points", xytext=(0, 30),
                ha="center", fontsize=8.5, color=C_BAD, fontweight="bold")
    # right: phase-robust kernel is immortal -> bar runs off the top
    ax.bar(1, top, color=C_OK, width=0.5, hatch="//", edgecolor="#2f6b28",
           alpha=0.85, zorder=2)
    ax.annotate("", xy=(1, top * 1.10), xytext=(1, top * 0.98),
                arrowprops=dict(arrowstyle="-|>", color="#2f6b28", lw=2))
    ax.annotate("immortal — ∞\n0 deaths\n(6M-frame soak)",
                (1, top * 0.5), ha="center", va="center", fontsize=10,
                color="white", fontweight="bold")

    ax.set_ylim(0, top * 1.18)
    ax.set_xlim(-0.6, 1.6)
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["single-phase kernel\n(idealised px = 149)",
                        "phase-robust kernel\n(px ∈ [146, 154])"])
    ax.set_ylabel(f"pipes cleared before death\n(always-flap; single-phase run to {single_cap:,} frames)")
    ax.set_title("Why phase-robustness matters: the idealised-cadence kernel is\n"
                 "immortal on the sandbox but DIES in the real game", fontsize=10)
    fig.tight_layout()
...
def fig_trajectory(R, seed=3, frames=430):
    ...
    traj = r["traj"]
    fig, (ax, ax2) = plt.subplots(2, 1, figsize=(9, 5.6),
    ...
    for t in traj:
        for wx, gap_top, gap_bot in t["pipes"]:
            ...
            if key in seen:
                continue
            seen.add(key)
            ax.add_patch(Rectangle((wx, -5), PIPE_W, gap_top + 5, color=C_OK, alpha=0.30, lw=0))
            ax.add_patch(Rectangle((wx, gap_bot), PIPE_W, BASEY - gap_bot,
                                   color=C_OK, alpha=0.30, lw=0))
    ...
    if ov:
        ax.scatter([p[0] for p in ov], [p[1] for p in ov], color=C_BAD, s=12,
                   zorder=5, label="shield overrode the proposal")
...
def fig_override_heatmap(R, frames, seeds):
    ...
    im = ax.imshow(rate, aspect="auto", extent=extent, cmap="magma_r",
                   vmin=0, vmax=1, interpolation="nearest")
    ax.contour(np.arange(V.V_MIN, V.V_MAX + 1), np.arange(Ny), R.astype(float),
               levels=[0.5], colors=C_SAFE, linewidths=1.8)
    cb = fig.colorbar(im, ax=ax)
    cb.set_label("shield override rate in this state")
    ax.set_title("Where the shield actually intervenes (random proposals)\n"
                 "blue line = kernel R boundary · grey = states never visited", fontsize=10)
    ax.set_xlabel("bird velocity (px/frame)")
    ax.set_ylabel("bird y, top-left (px, down ↓)")
...
def fig_latency(R, frames, seed):
    ...
    for val, lab, col in [(med, f"median {med:.2f} ms", "#333333"),
                          (p99, f"p99 {p99:.2f} ms", C_OUT),
                          (mx, f"max {mx:.2f} ms", C_BAD)]:
        ax.axvline(val, color=col, ls="--", lw=1.4)
        ax.text(val, ax.get_ylim()[1] * 0.9, "  " + lab, color=col, fontsize=8.5,
                rotation=90, va="top")
    ax.set_xlabel("shield decision time per frame (ms)")
    ax.set_ylabel(f"frames ({len(t):,} total)")
    ax.set_title(f"Deployed cost: {med:.2f} ms median, {mx:.2f} ms worst case —\n"
                 f"vs a {budget:.0f} ms/frame budget at 30 FPS "
                 f"({budget / mx:.0f}× headroom even at the worst frame)", fontsize=10)
    fig.tight_layout()
...
def main(which="all"):
    ...
    if which in ("all", "phase"):
        ...
        print(f"     phase-robust kernel: {robust_ok}/{len(seeds)} seeds immortal (capped)")
    ...
    if which in ("all", "survival"):
        ...
        print("  ->", fig_survival())
    print("done. (for the animated GIF from the real renderer: "
